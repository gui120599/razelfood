...
`env('X', 5)` devolve a STRING '5' quando a var está no .env (só o default fica int). Sob Carbon 3 / Symfony 8, string em `now()->addMinutes(...)`, `addDays(...)` etc. lança TypeError — foi o que derrubou todo o cardápio de tenant com HTTP 500 (config/tenancy.php `ttl_minutes` sem cast, usado em `ResolveTenantFromPath`). Sempre `(int)`/`(bool)`/`(float)` em valores de config alimentados por `env()` que serão usados como número/booleano. Ver tests/Feature/Tenancy/ResolveTenantFromPathCacheTtlTest.php.
