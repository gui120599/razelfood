...
- `CreateAction->using()` faz o loop: 1 registro por bairro via `$zone->neighborhoods()->create(['city_id' => ..., 'city' => ..., 'neighborhood' => ...])` (o mutator de DeliveryZoneNeighborhood normaliza city/neighborhood; `tenant_id` vem do BelongsToTenant). Retorna o último Model criado.
- Set/2026: `delivery_zone_neighborhoods.city_id` (FK nullable → `cities`, backfill best-effort na migration). Os `<select>` continuam keyed por `normalized_name`; só o SAVE (`using()` + `EditAction->mutateFormDataUsing()`) resolve `City::where('normalized_name', $data['city'])->value('id')` e grava `city_id`. `city` (string normalizada) NÃO saiu — continua sendo a fonte de `ResolveDeliveryFee`. O `city_id` serve pro checkout listar as cidades atendidas pelo tenant (`Checkout::servedCities`, ver [[app-livewire]]). Relação no model: `cityRecord()` (não `city()`, que é o Attribute string). Teste assere `city_id` no `assertDatabaseHas`.
