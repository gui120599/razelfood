...
echo "    No servidor:  cd ~/public_html && git pull origin producao"
echo "    Depois:       rm -f bootstrap/cache/{packages,services,config,routes-v7}.php"
echo "                  php artisan migrate --force && php artisan config:cache route:cache view:cache event:cache"
echo "                  php artisan filament:optimize && php artisan queue:restart"
